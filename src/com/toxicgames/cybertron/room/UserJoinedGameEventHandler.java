...
public class UserJoinedGameEventHandler extends BaseServerEventHandler
{
	...
	public void handleServerEvent(ISFSEvent event) throws SFSException
	{
		...
		user.setProperty(UserProps.POSX, posX);
		user.setProperty(UserProps.POSY, posY);

		int color = (int) (Math.random() * 0xFFFFFF);
		user.setProperty(UserProps.COLOR, color);

		user.setProperty(UserProps.X_DIRECTION, Math.random() > .5);
		user.setProperty(UserProps.Y_DIRECTION, Math.random() > .5);

		UserVariable posXVar = new SFSUserVariable("posX", posX);
		UserVariable posYVar = new SFSUserVariable("posY", posY);
		UserVariable colorVar = new SFSUserVariable("color", color);
